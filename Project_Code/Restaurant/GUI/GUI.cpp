...
//////////////////////////////////////////////////////////////////////////////////////////
void GUI::PrintAvailable(int N, int G, int V, int cN,int cG, int cV, int timestep, string* msgs, int amsg, int sN, int sG, int sV) const	//Prints a message on status bar
{
	this->PrintMessage(to_string(timestep));
	pWind->SetPen(DARKRED);
	pWind->SetFont(18, BOLD, BY_NAME, "Arial");
	string msg = "Number Of Waiting Orders [N,G,V] = [" + to_string(N) + ", " + to_string(G) + ", " + to_string(V) + "]";
	pWind->DrawString(800, WindHeight - (int)(StatusBarHeight / 1.1), msg);
	/*msg = "Waiting Vegan Orders = " + to_string(G);
	pWind->DrawString(800, WindHeight - (int)(StatusBarHeight / 1.3), msg);
	msg = "Waiting VIP Orders = " + to_string(V);
	pWind->DrawString(800, WindHeight - (int)(StatusBarHeight / 1.6), msg);
	*/
	msg = "Number Of Available Cooks [N, G, V] = [" + to_string(cN) + ", " + to_string(cG) + ", " + to_string(cV) + "]";
	pWind->DrawString(800, WindHeight - (int)(StatusBarHeight / 1.6), msg);

	msg = "Number Of Served Orders [N, G, V] = [" + to_string(sN) + ", " + to_string(sG) + ", " + to_string(sV) + "]";
	pWind->DrawString(800, WindHeight - (int)(StatusBarHeight / 1.3), msg);
	msg = "Types and IDs: ";
	//pWind->DrawString(800, WindHeight - (int)(StatusBarHeight / 2.5), msg);
	for (int i = 0; i <= amsg; i++) 
		msg = msg + msgs[i];
	pWind->DrawString(800, WindHeight - (int)(StatusBarHeight / 2.2), msg);
}
...
void GUI::AddToDrawingList(Order* pOrd)
{
	...
	/*
	if (reg != DONE_REG) {
		for (int i = cookcount; i <= DrawingItemsCount; i++) {
			for (int j = i + 1; j <= DrawingItemsCount; j++) {
				if (DrawingList[j]->ID < DrawingList[i]->ID) {
					DrawingItem* ptemp = new DrawingItem;
					ptemp = DrawingList[i];
					DrawingList[i] = DrawingList[j];
					DrawingList[j] = ptemp;
				}
			}
		}
